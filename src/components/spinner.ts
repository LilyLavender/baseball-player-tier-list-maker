...
export function renderSpinner(
  label = "Loading",
  size?: "sm" | "lg",
  trackVariant?: "current",
): string {
  const sizeClass = size ? ` spinner--${size}` : "";
  const trackClass = trackVariant === "current" ? " spinner--track-current" : "";
  return `
    <span class="spinner spinner--dual-chasers${sizeClass}${trackClass}" role="status" aria-label="${label}">
